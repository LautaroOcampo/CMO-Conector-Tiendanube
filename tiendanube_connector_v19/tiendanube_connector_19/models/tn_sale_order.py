...
class TNSaleOrder(models.Model):
    ...
    def _get_or_create_customer(self):
        ...
        - Sin provincia de entrega: contacto ``consumidor_final_partner_id`` (sin crear uno nuevo).
        - Con provincia: match por ``tn_customer_id`` (``customer.id`` del payload); si no hay coincidencia
          y hay nombre, apellido y provincia, se crea un ``res.partner`` con ese ID; si faltan datos mínimos,
          se usa el contacto Consumidor Final si está configurado.
        """
        self.ensure_one()
        config = self.tn_config_id or self.env['tn.config'].get_config()
        ...
        province_ok = bool((self.shipping_state or '').strip())
        minimum_for_new = bool(first_name and last_name and province_ok)

        tn_id = (self.tn_customer_id or '').strip()
        if tn_id:
            partner = self.env['res.partner'].search([
                ('tn_customer_id', '=', tn_id),
                '|',
                ('company_id', '=', False),
                ('company_id', '=', self.env.company.id),
            ], limit=1)
            if partner:
                tag = self.env['res.partner.category'].search([('name', '=', 'Cliente TN')], limit=1)
                if not tag:
                    tag = self.env['res.partner.category'].create({'name': 'Cliente TN'})
                update_vals = {}
                if tag and tag.id not in partner.category_id.ids:
                    update_vals['category_id'] = [(4, tag.id)]
                if self.customer_email and (not partner.email or not partner.email.strip()):
                    update_vals['email'] = self.customer_email.strip()
                ...
                _logger.info(
                    '✅ Cliente TN por tn_customer_id=%s: %s (id=%s)',
                    tn_id, partner.name, partner.id,
                )
                return partner
        ...
        if not minimum_for_new:
            ...
            if not partner:
                raise UserError(_(
                    'El pedido tiene provincia pero faltan nombre, apellido o datos incompletos para crear '
                    'un contacto nuevo, y no hay contacto "Consumidor Final" configurado en TiendaNube.'
                ))
            _logger.warning(
                '⚠️ TN Order %s: sin match por ID cliente y faltan nombre/apellido/provincia; '
                'usando Consumidor Final %s (id=%s).',
        ...
        _logger.info("✅ Cliente TN creado (nuevo customer id=%s): %s (ID: %s)", tn_id or '-', partner.name, partner.id)
        return partner
