...
class TNPublication(models.Model):
    ...
    description = fields.Html(string='Descripción', help='Descripción del producto', sanitize=False)
    ...
    def _apply_tn_values_wizard(self, wizard):
        """
        Aplica los valores del asistente en Odoo y ejecuta exportación completa a TiendaNube.
        Con varias variantes, el mismo precio, stock, SKU y código de barras se aplican a cada variante.
        """
        self.ensure_one()
        if not self.tn_product_id:
        ...
        if price <= 0:
            raise UserError(_('Indique un precio mayor a cero.'))
        ...
        v_vals = {'price': price, 'stock': stock}
        if sku_val:
            v_vals['sku'] = sku_val
        if barcode_val:
        ...
        message = _(
            'Datos enviados a TiendaNube. Si hay varias variantes, se aplicó el mismo precio, stock, SKU y código de barras a cada una.'
        )
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
