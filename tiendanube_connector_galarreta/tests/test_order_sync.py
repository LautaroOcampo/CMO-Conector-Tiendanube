...
class TestTNStoreCountry(TransactionCase):
    """País de tienda: fallback y etiqueta de envío sin llamar a la API."""

    def test_get_fallback_country_without_store_metadata(self):
        cfg = self.env['tn.config'].create({
            'name': 'País test',
            'store_id': '999001',
            'access_token': 'dummy',
            'company_id': self.env.company.id,
        })
        ar = self.env['res.country'].search([('code', '=', 'AR')], limit=1)
        fb = cfg.get_fallback_country_for_partners()
        if ar:
            self.assertEqual(fb, ar)

    def test_shipping_country_label_from_iso(self):
        cfg = self.env['tn.config'].create({
            'name': 'Label test',
            'store_id': '999002',
            'access_token': 'dummy',
        ...
        TNOrder = self.env['tn.sale.order']
        br = self.env['res.country'].search([('code', '=', 'BR')], limit=1)
        if br:
            label = TNOrder._tn_shipping_country_display_name('BR', cfg)
            self.assertEqual(label, br.name)
