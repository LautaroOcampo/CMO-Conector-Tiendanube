...
- **Qué hacer:** En `__manifest__.py`, en la lista `data`, colocar los XML de crons al final, por ejemplo después de todas las vistas: `'data/cron_stock_sync.xml'` y `'data/cron_order_sync.xml'` al final. Así se cargan primero modelos y vistas y después las acciones planificadas que referencian `model_tn_config` y `model_tn_publication`, reduciendo riesgo de errores en actualizaciones.
