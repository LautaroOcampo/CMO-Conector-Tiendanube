...
class TNPublication(models.Model):
    ...
    # Precio/stock de la publicación tal como figuran en la ficha (sincronizados con TiendaNube)
    current_price_tn = fields.Float(
        string='Precio (TiendaNube)',
        compute='_compute_current_values_tn',
        help='Precio de la publicación en TiendaNube: mínimo entre variantes con precio > 0.',
    )
    
    current_stock_tn = fields.Float(
        string='Stock (TiendaNube)',
        compute='_compute_current_values_tn',
        help='Stock de la publicación en TiendaNube: suma del stock de todas las variantes.',
    )
    
    # Campo para determinar si la sincronización automática está activa (para avisos en vista)
    has_auto_sync = fields.Boolean(
        string='Tiene Sincronización Automática',
        compute='_compute_has_auto_sync',
        help='Indica si la configuración tiene sincronización automática activada (se muestra un aviso en la ficha).',
    )
    ...
    @api.depends('variant_ids', 'variant_ids.price', 'variant_ids.stock')
    def _compute_current_values_tn(self):
        """Precio y stock de la publicación según las variantes (datos alineados con TiendaNube)."""
        for record in self:
            if not record.variant_ids:
                record.current_price_tn = 0.0
                record.current_stock_tn = 0.0
                continue
            prices = [v.price for v in record.variant_ids if v.price and v.price > 0]
            record.current_price_tn = min(prices) if prices else 0.0
            record.current_stock_tn = sum(record.variant_ids.mapped('stock')) or 0.0
    ...
    def write(self, vals):
        ...
        # Con tn_sku_link_only (matcheo por SKU) tampoco: ese flujo solo vincula.
        if 'odoo_product_id' in vals and not self.env.context.get('tn_sku_link_only'):
            record_ids = self.ids
    ...
    def action_import_from_tn(self, tn_config=None):
        ...
        try:
            ...
            return tn_config.action_import_publications()
    ...
    def _action_sync_all_to_tiendanube(self, sync_stock=True, sync_price=True):
        ...
        try:
            ...
            # Publicadas en TN + con producto Odoo.
            # Mismo criterio que la vista previa (action_view_stock_sync_publications).
            publications = self.env['tn.publication']
            for config in self.env['tn.config'].search([]):
                publications |= config._tn_get_stock_sync_publications()
            ...
            sync_service = self.env['tn.sync.service'].with_context(tn_sync_manual=True)
            synced_count = 0
            error_count = 0
            ...
            for publication in publications:
                try:
                    ...
                    # Actualizar stock desde Odoo (el precio a TN solo se cambia desde «Actualizar valores»)
                    publication._sync_stock_and_price_from_odoo(update_price=False)
                    
                    # Forzar guardado de las variantes para asegurar que el stock se guarde
            ...
            if not self.variant_ids or len(self.variant_ids) == 0:
                if not self.sku or not self.sku.strip():
                    ...
                    if odoo_template:
                        ...
                        self.with_context(tn_sku_link_only=True).write({'odoo_product_id': odoo_template.id})
                        linked_products += 1
                        ...
                        if not publication.sku or not publication.sku.strip():
                            ...
                            if odoo_template:
                                publication.with_context(tn_sku_link_only=True).write(
                                    {'odoo_product_id': odoo_template.id}
                                )
                                linked_products += 1
                            else:
                                not_found_variants += 1
    ...
    def action_export_to_tn(self):
        """Exporta/Publica la publicación en TiendaNube y deja el ID TN en el formulario."""
        self.ensure_one()
        try:
            _logger.info("[TN BIND] action_export_to_tn start pub=%s tn_product_id=%s", self.id, self.tn_product_id)
            sync_service = self.env['tn.sync.service'].with_context(tn_sync_manual=True)
            result = sync_service.export_publication(self)
            tn_id = (result or {}).get('product_id')
            _logger.info("[TN BIND] action_export_to_tn result=%s", result)
            self.invalidate_recordset(['tn_product_id', 'sync_state'])
            current_id = self.sudo().tn_product_id
            if tn_id and not current_id:
                _logger.warning("[TN BIND] result trae id=%s pero el form sigue vacío; reescribimos", tn_id)
                sync_service._bind_publication_to_tn_product(self, tn_id)
                self.invalidate_recordset(['tn_product_id', 'sync_state'])
                current_id = self.sudo().tn_product_id
            if not current_id:
                _logger.error("[TN BIND] action_export_to_tn SIN ID pub=%s result=%s", self.id, result)
                raise UserError(_(
                    'La exportación no guardó el ID de TiendaNube en la publicación.\n'
                    'Busque en el log de Odoo las líneas [TN BIND] y [TN EXPORT].'
                ))
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    ...
                    'message': _('Publicación ligada a TiendaNube (ID %s). Ya puede actualizar stock y valores.') % current_id,
                    'type': 'success',
                    'sticky': False,
                    'next': {
            ...
            _logger.exception("[TN BIND] Error al exportar publicación a TiendaNube: %s", e)
    ...
    def _sync_stock_and_price_from_odoo(self, update_price=False):
        """Sincroniza stock de las variantes desde Odoo hacia la publicación TN (sin precio)."""
        self.ensure_one()
        if not self.odoo_product_id:
            ...
            try:
                ...
                if self.tn_product_id:
                    try:
                        ...
                        sync_service.sync_published_status_to_tn(self)
    ...
    def action_open_tn_values_wizard(self):
        """Abre el asistente para enviar título, precio, stock, SKU, etc. a TiendaNube."""
        self.ensure_one()
        if not self.tn_product_id:
            raise UserError(_('La publicación aún no existe en TiendaNube. Use "Exportar a TiendaNube" primero.'))
        return {
            'type': 'ir.actions.act_window',
            'name': _('Actualizar valores'),
            'res_model': 'tn.publication.values.wizard',
            'view_mode': 'form',
            'target': 'new',
            'context': {'default_publication_id': self.id},
        }

    def _apply_tn_values_wizard(self, wizard):
        """
        Aplica los valores del asistente en Odoo y exporta a TiendaNube (sin precio desde Odoo).
        """
        self.ensure_one()
        if not self.tn_product_id:
            raise UserError(_('La publicación no tiene ID de producto en TiendaNube.'))
        price = float(wizard.price or 0.0)
        stock = float(wizard.stock or 0.0)
        if stock < 0:
            stock = 0.0
        title = (wizard.title_tn or '').strip()
        if not title:
            raise UserError(_('El título no puede estar vacío.'))
        sku_val = (wizard.sku or '').strip()
        barcode_val = (wizard.barcode or '').strip()
        marca_val = (wizard.marca or '').strip()

        self.write({
            'title': title,
            'description': wizard.description or False,
            'description_text': self._html_to_plain_text(wizard.description) if wizard.description else False,
            'marca': marca_val or False,
            'sku': sku_val or False,
            'barcode': barcode_val or False,
            'categoria_ids': [(6, 0, wizard.categoria_ids.ids)],
            'tag_ids': [(6, 0, wizard.tag_ids.ids)],
        })

        self.image_ids.unlink()
        seq = 10
        Image = self.env['tn.publication.image']
        for line in wizard.image_line_ids.sorted(key=lambda r: (r.sequence, r.id)):
            Image.create({
                'publication_id': self.id,
                'name': line.name or _('Imagen'),
                'position': int(line.sequence or seq),
                'image': line.image,
            })
            seq += 10
        ...
        if sku_val:
            v_vals['sku'] = sku_val
        if barcode_val:
            v_vals['barcode'] = barcode_val
        if self.variant_ids:
            self.variant_ids.write(v_vals)

        self.flush_recordset()
        self.variant_ids.flush_recordset(['price', 'stock', 'sku', 'barcode'])
        ...
        try:
            ...
            _logger.warning('No se pudo verificar pausa/despausa por stock tras exportar: %s', e, exc_info=True)

        message = _(
            'Datos enviados a TiendaNube (título, stock, SKU, etc.). '
            'El precio no se envía desde Odoo.'
        )
        if price > 0:
            message += _(' Precio guardado solo en Odoo: %.2f') % price
        return {
            'type': 'ir.actions.client',
            'tag': 'display_notification',
            'params': {
    ...
    def _html_to_plain_text(self, html_content):
        ...
        try:
            ...
            return (html2plaintext(html_content) or '').strip() or False
        except Exception:
            return False
