...
class TNSyncService(models.Model):
    ...
    def _get_config(self, tn_config=None):
        ...
        if tn_config:
            return {
                'client_id': tn_config.client_id or '',
                'client_secret': tn_config.client_secret or '',
                'access_token': tn_config.access_token or '',
                'refresh_token': tn_config.refresh_token or '',
                'store_id': tn_config.store_id or '',
                'api_base_url': tn_config.api_base_url or 'https://api.tiendanube.com/v1',
    ...
    def import_publications(self, limit=50, tn_config=None):
        """
        Importa publicaciones desde TiendaNube
        
        Args:
            limit: Número máximo de publicaciones a importar
            tn_config: Registro de tn.config a usar. Si no se proporciona, se usa la configuración global.
        """
        try:
            publications = []
            page = 1
            per_page = min(limit, 50)  # Máximo 50 por página según API
            ...
            while len(publications) < limit:
                ...
                # La API puede devolver los productos directamente o en un campo 'products'
                # TiendaNube API v1 devuelve una lista directamente
                if isinstance(response, list):
                    products = response
                elif isinstance(response, dict):
                    products = response.get('products', []) or response.get('data', [])
                else:
                    products = []
                ...
                for product_data in products:
                    ...
                    publication = self._create_publication_from_tn_data(product_data, tn_config=tn_config)
                    if publication:
                        publications.append(publication)
                
                # Si hay menos productos que el límite, no hay más páginas
    ...
    def _create_publication_from_tn_data(self, product_data, tn_config=None):
        ...
        try:
            ...
            # Intentar obtener collections desde la API si no vienen en los datos
            # Nota: El endpoint /products/{id}/collections no existe, pero podemos intentar obtener todas las collections
            # y verificar si el producto está en alguna de ellas
            if not product_data.get('collections') and not product_data.get('collection_ids'):
                try:
                    # Intentar obtener todas las collections y buscar si el producto está en alguna
                    collections_response = self._make_request('GET', 'collections', retry=False, tn_config=tn_config)
                    if collections_response and isinstance(collections_response, list):
                        # Filtrar collections que contengan este producto
                        product_collections = []
                        for collection in collections_response:
                            if isinstance(collection, dict):
                                ...
                                if collection_id:
                                    try:
                                        # Intentar obtener productos de esta collection
                                        collection_products = self._make_request('GET', f'collections/{collection_id}/products', retry=False, tn_config=tn_config)
                                        if collection_products and isinstance(collection_products, list):
                                            product_ids = [p.get('id') if isinstance(p, dict) else p for p in collection_products]
                                            if tn_product_id in product_ids:
                                                product_collections.append(collection)
                                                _logger.info("✅ Producto encontrado en collection: %s (ID: %s)", 
                                                            collection.get('name', ''), collection_id)
                                    except Exception:
                                        pass
                        
                        if product_collections:
                            product_data['collections'] = product_collections
                            _logger.info("✅ Collections encontradas para el producto: %s", len(product_collections))
                except Exception as e:
                    _logger.warning("⚠️ No se pudieron obtener collections desde API: %s", e)
            ...
            if tn_product_id:
                try:
                    ...
                    if full_product and isinstance(full_product, dict):
                        # Usar producto completo (tiene variants); conservar collections si ya se habían cargado
                        collections_backup = product_data.get('collections') or product_data.get('collection_ids')
                        product_data = dict(full_product)
                        if collections_backup:
                            product_data['collections'] = collections_backup
            ...
            if import_at_variant_level:
                ...
                for item in real_variants_list:
                    ...
                    self._import_images(publication, product_data.get('images', []))
                    try:
    ...
    def _import_images(self, publication, images_data):
        """Importa imágenes desde TiendaNube"""
    ...
    def _sync_to_odoo_product(self, publication, product_data):
        ...
        Matcheo solo por default_code (Referencia interna). Si no hay coincidencia, no se relaciona."""
        try:
            ...
            # Solo actualizar si existe, NO crear productos nuevos
            if product_template:
                ...
                if product_data.get('variants'):
                    first_variant = product_data['variants'][0]
                    product_vals['list_price'] = float(first_variant.get('price', 0))
    ...
    def _link_publication_variants_to_odoo_by_sku(self, publication):
        ...
        for variant in publication.variant_ids:
            ...
            if not publication.odoo_product_id:
                publication.odoo_product_id = odoo_variant.product_tmpl_id
    ...
    def export_publication(self, publication):
        ...
        try:
            # Obtener credenciales desde la cuenta de la publicación
            ...
            if publication.tn_product_id:
                ...
                product_id = publication.tn_product_id
            else:
                ...
                # respuesta puede venir en distintos formatos
                if isinstance(response, dict):
                    product_id = response.get('id') or response.get('product', {}).get('id')
            ...
            _logger.info("🟩 Respuesta TiendaNube (producto): %s", response)
            ...
            try:
                ...
                self._update_variants_individually(publication, product_id)
            except Exception as e:
    ...
    def _prepare_variants(self, publication):
        ...
        for variant in publication.variant_ids:
            # Construir variante siguiendo formato exacto de la documentación
            v = {}
            
            # price: OBLIGATORIO que sea NUMÉRICO (float o int), NUNCA string
            # TiendaNube API requiere price como número, no como string
            try:
                if variant.price is not None:
                    price_value = float(variant.price)
                    if price_value < 0:
                        _logger.warning("⚠️ Variante '%s' tiene precio negativo (%s), usando 0.0", 
                                      variant.name or 'sin nombre', price_value)
                        price_value = 0.0
                else:
                    price_value = 0.0
                
                # Asegurar que price sea float (numérico), NUNCA string
                v["price"] = float(price_value)
                
                # Log del precio con su tipo
                _logger.info("💰 Variante '%s': price=%s (tipo: %s)", 
                           variant.name or 'sin nombre', 
                           v["price"], 
                           type(v["price"]).__name__)
            except (ValueError, TypeError) as e:
                _logger.error("❌ ERROR: Variante '%s' tiene precio inválido: %s (tipo: %s). Error: %s", 
                            variant.name or 'sin nombre', 
                            variant.price, 
                            type(variant.price).__name__,
                            str(e))
                # Usar 0.0 como fallback
                v["price"] = 0.0
                _logger.warning("⚠️ Usando precio por defecto 0.0 para variante '%s'", variant.name or 'sin nombre')
    ...
    def _update_variants_individually(self, publication, product_id, sync_stock=True, sync_price=True):
        """
        Actualiza variantes ya existentes en TiendaNube usando:
        PUT /products/{product_id}/variants

        Basado en la documentación oficial de Product Variant:
        https://tiendanube.github.io/api-documentation/resources/product-variant

        Se envía un array de variantes, cada una con su 'id' y los campos a actualizar
        (price, sku, values, weight, stock opcional).
        
        Args:
            publication: Publicación de TiendaNube
        ...
        if publication.main_tn_variant_id and variants_with_tn_id:
            ...
            for variant in publication.variant_ids:
                ...
                if sync_price:
                    payload["price"] = float(variant.price) if variant.price is not None else 0.0
            ...
            try:
                ...
                if response and isinstance(response, dict):
                    ...
                    if len(api_variants) == 1:
                        ...
                        # Precio (necesario) - como número, no string
                        # Si sync_price es False, mantener el precio actual de TiendaNube
                        if sync_price:
                            if publication.variant_ids and publication.variant_ids[0].price:
                                virtual_payload["price"] = float(publication.variant_ids[0].price)
                            else:
                                virtual_payload["price"] = 0.0
                        else:
                            # Obtener precio actual de TiendaNube para no cambiarlo
                            if api_variants and len(api_variants) > 0:
                                current_price = api_variants[0].get('price', 0.0)
                                virtual_payload["price"] = float(current_price)
                                _logger.info("   ℹ️ Precio no sincronizado, manteniendo precio actual de TiendaNube: %s", current_price)
                            else:
                                virtual_payload["price"] = 0.0
        ...
        for variant in publication.variant_ids:
            ...
            price_value = float(variant.price) if variant.price is not None else 0.0
            sku_value = variant.sku or ''

            v = {
                "id": variant.tn_variant_id,
                "price": price_value,  # Precio como número, no string
            }
